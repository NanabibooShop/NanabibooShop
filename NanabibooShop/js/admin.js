...
(function () {
  ...
  function markDirty() {
    dirty = true;
    $("#dirty").hidden = false;
    try { localStorage.setItem(DRAFT_KEY, JSON.stringify({ at: Date.now(), shop, costumes: items.map(({ _origId, ...c }) => c) })); } catch (e) {}
  }
  function markClean() {
    dirty = false;
    $("#dirty").hidden = true;
  ...
  $$("[data-shop]").forEach((el) => el.addEventListener("input", () => {
    ...
    else if (k === "holdMinutes") shop[k] = Math.max(5, Math.min(170, parseInt(el.value, 10) || 30));
  ...
  function statusBadge(c) {
    ...
    if (a.state === "busy") return `<span class="row__badge is-busy">Đang thuê</span>`;
  ...
  function renderEditor() {
      ...
      <section class="panel" id="schedPanel">
        ...
        <div class="lock-box">
          ...
          <div class="legend">
            ...
            ${MODE === "firebase" ? `<span><i class="lg-held"></i>Khách đang giữ chỗ</span>` : ""}
    ...
    del.onclick = () => {
      if (del.dataset.armed) {
        if (c._origId) deletedIds.add(c._origId);
  ...
  function renderMini(c) {
    ...
    for (let m = 0; m < 3; m++) {
      ...
      for (let d = 1; d <= nDays; d++) {
        ...
        const held = !booked && holdFor(c, day);
        const cls = [
          booked ? "b" : held ? "h" : window.NB.cleaningOn(c, day, clean) ? "c" : st === "free" ? "f" : "x",
          +day === +t ? "t" : "", past ? "p" : "",
...
`;
  ...
  $("#saveBtn").onclick = async () => {
    ...
    if (MODE === "firebase") { await saveOnline(); return; }
  ...
  /* ---------- Firebase: lưu lên mạng ---------- */
  async function saveOnline() {
    const btn = $("#saveBtn");
    btn.disabled = true; btn.textContent = "Đang lưu…";
    try {
      items.forEach((c) => { if (c._origId && c._origId !== c.id) deletedIds.add(c._origId); });
      [...deletedIds].forEach((id) => { if (items.some((c) => c.id === id)) deletedIds.delete(id); });
      await API.saveAll(shop, items, [...deletedIds]);
      deletedIds.clear();
      items.forEach((c) => { c._origId = c.id; });
      ORIGINAL = snapshot();
      markClean();
      $("#firstBanner") && ($("#firstBanner").hidden = true);
      toast("Đã lưu ✓ — trang khách cập nhật ngay.");
      if (pending.size) askSaveImages();
    } catch (e) {
      console.error(e);
      toast(/permission/i.test(String(e && (e.code || e.message))) ? "Tài khoản này chưa có quyền sửa — kiểm tra email trong Firestore Rules." : "Không lưu được: " + (e && e.message ? e.message : e));
    } finally {
      btn.disabled = false; btn.textContent = "Lưu";
    }
  }
  function askSaveImages() {
    const imgs = [...pending.keys()];
    const canFs = "showDirectoryPicker" in window;
    $("#saveBody").innerHTML = `
      <div class="save-opts">
        <p class="hint">Thông tin đã lên web. Riêng <b>${imgs.length} ảnh/clip mới</b> cần nằm trong thư mục <code>images/</code> của shop rồi upload lên GitHub thì khách mới thấy:</p>
        <ul class="save-list">${imgs.map((p) => `<li><code>${esc(p)}</code> ← ${esc(pending.get(p).name)}</li>`).join("")}</ul>
        ${canFs ? `<button class="btn btn--primary btn--sm" type="button" id="saveFs">${dirHandle ? "Lưu ảnh vào " + esc(dirHandle.name) : "Chọn thư mục shop & lưu ảnh"}</button>` : `<p class="hint">Máy này không tự lưu ảnh được — chép tay các ảnh trên vào thư mục <code>images/</code> với đúng tên.</p>`}
        <p class="hint">Mẹo: dán link ảnh/clip (TikTok, YouTube, ảnh đã đăng) thì không cần upload.</p>
      </div>`;
    saveSheet.showModal();
    if (canFs) $("#saveFs").onclick = saveToFolder;
  }

  /* ---------- Firebase: khách đang giữ chỗ ---------- */
  function holdFor(c, day) {
    const k = iso(day), now = Date.now();
    return liveHolds.find((h) => h.costumeId === c.id && h.expiresAt > now && k >= h.from && k <= h.to);
  }
  function renderHolds() {
    const panel = $("#holdsPanel");
    if (MODE !== "firebase") { panel.hidden = true; return; }
    panel.hidden = false;
    const now = Date.now();
    const q = ($("#holdQ").value || "").trim().toUpperCase();
    const act = liveHolds.filter((h) => h.expiresAt > now);
    $("#holdsN").textContent = act.length;
    const list = liveHolds.filter((h) => !q || h.code.includes(q)).sort((a, b) => (b.expiresAt > now) - (a.expiresAt > now) || a.expiresAt - b.expiresAt);
    $("#holdsList").innerHTML = list.length ? list.map((h) => {
      const c = items.find((x) => x.id === h.costumeId);
      const left = Math.ceil((h.expiresAt - now) / 60000);
      const f = parse(h.from), t = parse(h.to);
      return `<div class="hold ${left > 0 ? "" : "is-over"}">
        ...
        <div class="hold__act">
          ${left > 0 ? `<button class="btn btn--primary btn--sm" type="button" data-hok="${esc(h.code)}">Xác nhận thuê</button>` : ""}
          <button class="btn btn--ghost btn--sm" type="button" data-hno="${esc(h.code)}">${left > 0 ? "Huỷ" : "Xoá"}</button>
        </div>
      </div>`;
    }).join("") : `<p class="hint">${q ? "Không thấy mã này." : "Chưa có khách nào đang giữ chỗ."}</p>`;
    $$("[data-hok]").forEach((b) => b.onclick = async () => {
      const h = liveHolds.find((x) => x.code === b.dataset.hok); if (!h) return;
      b.disabled = true; b.textContent = "Đang khoá…";
      try {
        const note = "Mã " + h.code;
        ...
        renderList();
        toast(`Đã khoá lịch ${fmt(parse(h.from))} → ${fmt(parse(h.to))} cho mã ${h.code}.`);
      } catch (e) { console.error(e); toast("Không xác nhận được: " + (e.message || e)); b.disabled = false; b.textContent = "Xác nhận thuê"; }
    });
    $$("[data-hno]").forEach((b) => b.onclick = async () => {
      const h = liveHolds.find((x) => x.code === b.dataset.hno); if (!h) return;
      if (!b.dataset.armed && h.expiresAt > Date.now()) {
        b.dataset.armed = "1"; b.textContent = "Bấm lần nữa để huỷ";
        setTimeout(() => { if (b.isConnected) { delete b.dataset.armed; b.textContent = "Huỷ"; } }, 3000);
        return;
      }
      try { await API.cancelHold(h); toast("Đã huỷ giữ chỗ " + h.code + " — lịch mở lại."); }
      catch (e) { console.error(e); toast("Không huỷ được: " + (e.message || e)); }
    });
  }
  $("#holdQ").addEventListener("input", renderHolds);
  setInterval(() => { if (MODE === "firebase") { renderHolds(); if (items[sel]) renderMini(items[sel]); } }, 30000);
  ...
  $("#backupBtn").onclick = () => {
    ...
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(a.href), 2000);
    toast("Đã tải data.js — chép đè vào thư mục shop rồi upload lên GitHub.");
  };
  ...
  /* ---------- Khởi động ---------- */
  function snapshot() { return JSON.stringify({ shop, costumes: items.map(({ _origId, ...c }) => c) }); }
  function init(shopData, costumesData, opts) {
    ...
    renderList();
    renderEditor();
    adm.dataset.view = matchMedia("(min-width: 900px)").matches ? "edit" : "list";
    checkDraft();
  }
  ...
  function startFirebase(api) {
    ...
    $("#loginForm").addEventListener("submit", async (e) => {
      ...
      try { await api.signIn($("#lgEmail").value.trim(), $("#lgPass").value); }
      catch (err) { $("#lgErr").textContent = /invalid|wrong|user-not-found|credential/i.test(err.code || "") ? "Sai email hoặc mật khẩu." : "Không đăng nhập được: " + (err.code || err.message); }
      finally { $("#lgBtn").disabled = false; }
    });
    $("#lgReset").onclick = async () => {
      const email = $("#lgEmail").value.trim();
      if (!email) { $("#lgErr").textContent = "Nhập email trước rồi bấm Quên mật khẩu."; return; }
      try { await api.resetPassword(email); $("#lgErr").textContent = "Đã gửi email đặt lại mật khẩu."; } catch (e) { $("#lgErr").textContent = "Không gửi được: " + (e.code || e.message); }
    };
    $("#logoutBtn").onclick = async () => {
      if (dirty && !$("#logoutBtn").dataset.armed) { $("#logoutBtn").dataset.armed = "1"; $("#logoutBtn").textContent = "Còn thay đổi chưa lưu — bấm lần nữa"; return; }
      delete $("#logoutBtn").dataset.armed; $("#logoutBtn").textContent = "Đăng xuất";
      await api.signOut();
    };
    api.onAuth(async (user) => {
      ...
      box.hidden = true; adm.hidden = false; $("#logoutBtn").hidden = false; $("#liveBadge").hidden = false; $("#saveBtn").hidden = false;
      $("#liveBadge").title = "Đăng nhập: " + user.email;
      if (loadedFor === user.uid) return;
      loadedFor = user.uid;
      try {
        const r = await api.loadAll();
        if (!r.exists) {
          init(window.FILE_DATA.shop, window.FILE_DATA.costumes);
          const b = document.createElement("div");
          b.className = "banner"; b.id = "firstBanner";
          b.innerHTML = `<span><b>Firebase chưa có dữ liệu.</b> Đang hiển thị dữ liệu từ data.js — kiểm tra lại rồi bấm <b>Lưu</b> để đưa toàn bộ lên mạng.</span>`;
          adm.prepend(b);
          markDirty();
        } else init(r.shop, r.costumes, { fromDb: true });
      } catch (e) {
        console.error(e);
        toast("Không tải được dữ liệu: " + (e.code || e.message));
        return;
      }
  ...
  window.NBData.on((D) => {
    if (booted) return;
    booted = true;
    if (D.mode === "firebase" && D.api) startFirebase(D.api);
    else init(window.FILE_DATA.shop, window.FILE_DATA.costumes);
  });
})();
